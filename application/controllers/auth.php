...
class Auth extends CI_Controller {
	...
	function index()
	{
		...
		if (!$this->ion_auth->logged_in())
		{
			...
			redirect('uzytkownik/login', 'refresh');
		}
		...
		else
		{
			...
			$this->_render_page('templates/header_adm');
	...
	function login()
	{
		...
		else
		{
			...
			$this->data['bladlog'] = validation_errors();
			...
			// $this->_render_page('dynamic/v_mail', $this->data);
			
			// TU TRZEBA ZROBIC JAK¥Œ £ADN¥ STRONÊ Z INFORMACJ¥ O NIEPOPRAWNYM LOGOWANIU (SKOPIOWAÆ VIEW LOGIN.PHP), ALE PO DODANIU INFORMACJI O NIESPÓJNYM LOGINIE I HAŒLE
			$this->_render_page('auth/login', $this->data);
			//$redirect = str_replace('/bie/','',$_POST['redirect']);
			//if($redirect==''){$redirect='dynamic/v_mail';}
			//$this->_render_page($redirect, $this->data);
	...
	function logout()
	{
		...
		//redirect('welcome/bajo', 'refresh');
		
		//A TU JAK¥Œ £ADN¥ STRONKÊ O TYM, ¯E SIÊ WYLOGOWA£O
		redirect('/', 'refresh');
	}
	...
	function edit_user($id)
	{
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				//check to see if we are creating the user
				//redirect them back to the admin page
				$this->session->set_flashdata('message', "Zapisano zmiany");
				redirect("auth", 'refresh');
			}
		}
